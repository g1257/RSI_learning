...
int main(int argc, char* argv[])
{
	...
	// Example n = 3  ---> H = (1+g)[sx_0 sx_1 + sx_1 sx_2 + sx_2 sx_0] + (1-g)[sy_0 sy_1 + sy_1 sy_2 + sy_2 sy_0]
	// There are a identities implied!
	// sigmax acts only on one site: sx |0> = |1>, sx|1> = |0>, sy|0> = -i|1>, sy|1> = i|0>
	// 
	// Example H |alice> =(1+g) [|5> + |0> + |6>] + (1-g) [-i|1> i|0> |1> + |0> i|0> i|0> + -i|1> |1> i|0>   ] 
	//
	...
	for (unsigned int row = 0; row < hilbert; ++row) {
		...
		for (unsigned int site = 0; site < n; ++site) {
			...
			//row: bit (next_site) xor bit (site) 
			unsigned int r1 = row & (1<<site);
			unsigned int r2 = row & (1<<next_site);
			if (r1 > 0) {
				r1 = 1;
			}
			...
			hamiltonian(row, col) = g;

			} else {
			
			hamiltonian(row, col) = 1;
