...
int main(int argc, char* argv[])
{
	...
	//name n:number of sites m: 0 for even 1 for odd

	if (argc < 2){

	std::cerr << "Usage:" << argv[0] << " <sites>\n";
	return 1;

	}

	unsigned int n = std::atoi(argv[1]);
	// the initialzation doesn't look great, but I want to keep 1 odd
	for (unsigned int i = 4 - n; i < 11; i +=2){
